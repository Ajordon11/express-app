...
for (let i = 0; i < 10000; i++) {
    bigArray.push({id: i, title: 'title' + i, value: '' + Math.floor(Math.random() * 500)})
}
...
router.get('/data', (req, res) => {
        ...
        const filterId = req.query.search.value; // filter value (from search field)
        // console.log('filter ', filterId);
        const bigData = bigArray.filter(e => e.value.indexOf(filterId) > -1)
